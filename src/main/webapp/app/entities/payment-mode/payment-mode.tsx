...
import { getSearchEntities, getEntities } from './payment-mode.reducer';
import { IPaymentMode } from 'app/shared/model/payment-mode.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IPaymentModeProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IPaymentModeState {
  search: string;
}

export class PaymentMode extends React.Component<IPaymentModeProps, IPaymentModeState> {
  state: IPaymentModeState = {
    search: ''
  };
  ...
  render() {
    const { paymentModeList, match } = this.props;
    return (
      <div>
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('cargotrackerApp.paymentMode.home.search')}
                  />
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {paymentModeList.map((paymentMode, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${paymentMode.id}`} color="link" size="sm">
                      {paymentMode.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${paymentMode.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${paymentMode.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
...
const mapStateToProps = ({ paymentMode }: IRootState) => ({
  paymentModeList: paymentMode.entities
});
