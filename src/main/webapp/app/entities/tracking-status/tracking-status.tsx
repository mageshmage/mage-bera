...
import { getSearchEntities, getEntities } from './tracking-status.reducer';
import { ITrackingStatus } from 'app/shared/model/tracking-status.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface ITrackingStatusProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface ITrackingStatusState {
  search: string;
}

export class TrackingStatus extends React.Component<ITrackingStatusProps, ITrackingStatusState> {
  state: ITrackingStatusState = {
    search: ''
  };
  ...
  render() {
    const { trackingStatusList, match } = this.props;
    return (
      <div>
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('cargotrackerApp.trackingStatus.home.search')}
                  />
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {trackingStatusList.map((trackingStatus, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${trackingStatus.id}`} color="link" size="sm">
                      {trackingStatus.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${trackingStatus.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${trackingStatus.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
...
const mapStateToProps = ({ trackingStatus }: IRootState) => ({
  trackingStatusList: trackingStatus.entities
});
