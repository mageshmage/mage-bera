...
import { getSearchEntities, getEntities } from './shipment-mode.reducer';
import { IShipmentMode } from 'app/shared/model/shipment-mode.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';

export interface IShipmentModeProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export interface IShipmentModeState {
  search: string;
}

export class ShipmentMode extends React.Component<IShipmentModeProps, IShipmentModeState> {
  state: IShipmentModeState = {
    search: ''
  };
  ...
  render() {
    const { shipmentModeList, match } = this.props;
    return (
      <div>
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('cargotrackerApp.shipmentMode.home.search')}
                  />
        ...
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {shipmentModeList.map((shipmentMode, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${shipmentMode.id}`} color="link" size="sm">
                      {shipmentMode.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${shipmentMode.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${shipmentMode.id}/delete`} color="danger" size="sm">
                        <FontAwesomeIcon icon="trash" />{' '}
                        <span className="d-none d-md-inline">
...
const mapStateToProps = ({ shipmentMode }: IRootState) => ({
  shipmentModeList: shipmentMode.entities
});
