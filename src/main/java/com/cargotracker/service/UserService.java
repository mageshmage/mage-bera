...
import com.cargotracker.web.rest.errors.*;
...
public class UserService {
	...
	public User createUser(UserDTO userDTO) {
		...
		Vendor vendor = vendorRepository.findById(userDTO.getVendorId()).get();
	...
	public Optional<UserDTO> updateUser(UserDTO userDTO) {
		return Optional.of(userRepository.findById(userDTO.getId())).filter(Optional::isPresent).map(Optional::get)
				.map(user -> {
					...
					if (userExtraId != null) {
						userExtra = userExtraRepository.findById(userExtraId).get();
						// UserExtra userExtra = userExtraRepository.findOneByUser(user).get();
						if (userExtra != null && userDTO.getVendorId() != null) {
							Vendor vendor = vendorRepository.findById(userDTO.getVendorId()).get();
